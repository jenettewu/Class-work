...
def draw_button(button_text, button_x, button_y, button_width, button_height, (r, g, b)):
    global playing_pass, leaderboard_pass, instruction_pass, death, menu_screen
    noStroke()
...
def instruction():
    ...
    if instruction_pass == True:
        ...
        text(" Your goal is to eat as much as you can", 320, 220)
        text("without being eaten.", 320, 260)
...
def leaderboard():
    ...
    if leaderboard_pass == True:
        ...
        for i in range(len(total_games)):
            ...
            text("Game " + str(total_games[i]), 180, 110 + word_height*i)
            text("Score " + str(game_scores[i]), 320, 110 + word_height*i)
...
def other_fish():
    #background("#6EC5E9")
    global speed, o_fish_length, o_fish_width, player_fish_width, player_fish_length, death
    ...
    for fish_num in range(8):
        ...
        if (speed[fish_num] < 0 and o_fish_xco[fish_num] < -100) or (speed[fish_num] > 0 and o_fish_xco[fish_num] > 700):
            ...
            o_fish_colour[fish_num ] = (color(random(255), random(255), random(255)))
            if o_fish_xco[fish_num] == 0:
                o_fish_xco[fish_num] = int(random(-400, -40))
